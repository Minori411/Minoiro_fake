class Plan < ApplicationRecord
  ...
  def self.search(keyword, price)
    ...
    smallplan = []
    if price == "2"
      smallplan = Smallplan.where("price <= 2000")
    end

    if price == "3"
      smallplan = Smallplan.where("price <= 3000")
        end

    if price == "4"
      smallplan = Smallplan.where("price <= 4000")
    end

    if price == "5"
      smallplan = Smallplan.where("price <= 4999")
    end
    ...
    small_plan = smallplan.pluck(:plan_id) unless smallplan.blank?
    logger.warn("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
